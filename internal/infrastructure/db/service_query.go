...
func (sq *ServiceQuery) QueryClinic() *ClinicQuery {
	query := (&ClinicClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) QueryAppointments() *AppointmentQuery {
	query := (&AppointmentClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) QueryOrderItems() *OrderItemQuery {
	query := (&OrderItemClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withClinic:       sq.withClinic.Clone(),
		withAppointments: sq.withAppointments.Clone(),
		withOrderItems:   sq.withOrderItems.Clone(),
...
func (sq *ServiceQuery) WithClinic(opts ...func(*ClinicQuery)) *ServiceQuery {
	query := (&ClinicClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) WithAppointments(opts ...func(*AppointmentQuery)) *ServiceQuery {
	query := (&AppointmentClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withAppointments = query
	return sq
}
...
func (sq *ServiceQuery) WithOrderItems(opts ...func(*OrderItemQuery)) *ServiceQuery {
	query := (&OrderItemClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withOrderItems = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	var (
		...
		loadedTypes = [3]bool{
			sq.withClinic != nil,
			sq.withAppointments != nil,
			sq.withOrderItems != nil,
		}
	)
	if sq.withClinic != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withClinic; query != nil {
		if err := sq.loadClinic(ctx, query, nodes, nil,
			func(n *Service, e *Clinic) { n.Edges.Clinic = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withAppointments; query != nil {
		if err := sq.loadAppointments(ctx, query, nodes,
			func(n *Service) { n.Edges.Appointments = []*Appointment{} },
			func(n *Service, e *Appointment) { n.Edges.Appointments = append(n.Edges.Appointments, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withOrderItems; query != nil {
		if err := sq.loadOrderItems(ctx, query, nodes,
			func(n *Service) { n.Edges.OrderItems = []*OrderItem{} },
			func(n *Service, e *OrderItem) { n.Edges.OrderItems = append(n.Edges.OrderItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServiceQuery) loadClinic(ctx context.Context, query *ClinicQuery, nodes []*Service, init func(*Service), assign func(*Service, *Clinic)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Service)
	for i := range nodes {
		if nodes[i].clinic_services == nil {
			continue
		}
		fk := *nodes[i].clinic_services
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "clinic_services" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *ServiceQuery) loadAppointments(ctx context.Context, query *AppointmentQuery, nodes []*Service, init func(*Service), assign func(*Service, *Appointment)) error {
	...
	query.Where(predicate.Appointment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.AppointmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_appointments
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_appointments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_appointments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *ServiceQuery) loadOrderItems(ctx context.Context, query *OrderItemQuery, nodes []*Service, init func(*Service), assign func(*Service, *OrderItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrderItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.OrderItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_order_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_order_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_order_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
