...
func (oiq *OrderItemQuery) QueryService() *ServiceQuery {
	query := (&ServiceClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) QueryAppointment() *AppointmentQuery {
	query := (&AppointmentClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) Clone() *OrderItemQuery {
	...
	return &OrderItemQuery{
		...
		withService:     oiq.withService.Clone(),
		withAppointment: oiq.withAppointment.Clone(),
...
func (oiq *OrderItemQuery) WithService(opts ...func(*ServiceQuery)) *OrderItemQuery {
	query := (&ServiceClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withService = query
	return oiq
}
...
func (oiq *OrderItemQuery) WithAppointment(opts ...func(*AppointmentQuery)) *OrderItemQuery {
	query := (&AppointmentClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withAppointment = query
	return oiq
}
...
//		GroupBy(orderitem.FieldItemType).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.OrderItem.Query().
//		Select(orderitem.FieldItemType).
//		Scan(ctx, &v)
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			oiq.withProduct != nil,
			oiq.withService != nil,
			oiq.withAppointment != nil,
		}
	)
	if oiq.withOrder != nil || oiq.withProduct != nil || oiq.withService != nil || oiq.withAppointment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oiq.withService; query != nil {
		if err := oiq.loadService(ctx, query, nodes, nil,
			func(n *OrderItem, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := oiq.withAppointment; query != nil {
		if err := oiq.loadAppointment(ctx, query, nodes, nil,
			func(n *OrderItem, e *Appointment) { n.Edges.Appointment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Order)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_order_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Product)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_order_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Service)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OrderItem)
	for i := range nodes {
		if nodes[i].service_order_items == nil {
			continue
		}
		fk := *nodes[i].service_order_items
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_order_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) loadAppointment(ctx context.Context, query *AppointmentQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Appointment)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OrderItem)
	for i := range nodes {
		if nodes[i].appointment_order_items == nil {
			continue
		}
		fk := *nodes[i].appointment_order_items
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "appointment_order_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
