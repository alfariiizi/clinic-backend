...
func (brq *BillingRecordQuery) QueryClinic() *ClinicQuery {
	query := (&ClinicClient{config: brq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(billingrecord.Table, billingrecord.FieldID, selector),
...
func (brq *BillingRecordQuery) QueryPatient() *PatientQuery {
	query := (&PatientClient{config: brq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := brq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (brq *BillingRecordQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (brq *BillingRecordQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (brq *BillingRecordQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (brq *BillingRecordQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (brq *BillingRecordQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (brq *BillingRecordQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := brq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (brq *BillingRecordQuery) Clone() *BillingRecordQuery {
	...
	return &BillingRecordQuery{
		...
		withClinic:  brq.withClinic.Clone(),
		withPatient: brq.withPatient.Clone(),
...
func (brq *BillingRecordQuery) WithClinic(opts ...func(*ClinicQuery)) *BillingRecordQuery {
	query := (&ClinicClient{config: brq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	brq.withClinic = query
	return brq
}
...
func (brq *BillingRecordQuery) WithPatient(opts ...func(*PatientQuery)) *BillingRecordQuery {
	query := (&PatientClient{config: brq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	brq.withPatient = query
	return brq
}
...
//		GroupBy(billingrecord.FieldInvoiceNumber).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.BillingRecord.Query().
//		Select(billingrecord.FieldInvoiceNumber).
//		Scan(ctx, &v)
...
func (brq *BillingRecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*BillingRecord, error) {
	...
	if brq.withClinic != nil || brq.withPatient != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, billingrecord.ForeignKeys...)
	}
	...
	if query := brq.withClinic; query != nil {
		if err := brq.loadClinic(ctx, query, nodes, nil,
			func(n *BillingRecord, e *Clinic) { n.Edges.Clinic = e }); err != nil {
			return nil, err
		}
	}
	if query := brq.withPatient; query != nil {
		if err := brq.loadPatient(ctx, query, nodes, nil,
			func(n *BillingRecord, e *Patient) { n.Edges.Patient = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (brq *BillingRecordQuery) loadClinic(ctx context.Context, query *ClinicQuery, nodes []*BillingRecord, init func(*BillingRecord), assign func(*BillingRecord, *Clinic)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*BillingRecord)
	for i := range nodes {
		if nodes[i].clinic_billing_records == nil {
			continue
		}
		fk := *nodes[i].clinic_billing_records
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "clinic_billing_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (brq *BillingRecordQuery) loadPatient(ctx context.Context, query *PatientQuery, nodes []*BillingRecord, init func(*BillingRecord), assign func(*BillingRecord, *Patient)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*BillingRecord)
	for i := range nodes {
		if nodes[i].patient_billing_records == nil {
			continue
		}
		fk := *nodes[i].patient_billing_records
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "patient_billing_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (brq *BillingRecordQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(billingrecord.Table, billingrecord.Columns, sqlgraph.NewFieldSpec(billingrecord.FieldID, field.TypeUUID))
	_spec.From = brq.sql
