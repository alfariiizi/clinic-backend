...
func (oshq *OrderStatusHistoryQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (oshq *OrderStatusHistoryQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (oshq *OrderStatusHistoryQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (oshq *OrderStatusHistoryQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (oshq *OrderStatusHistoryQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (oshq *OrderStatusHistoryQuery) IDsX(ctx context.Context) []uuid.UUID {
...
//		GroupBy(orderstatushistory.FieldStatus).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.OrderStatusHistory.Query().
//		Select(orderstatushistory.FieldStatus).
//		Scan(ctx, &v)
...
func (oshq *OrderStatusHistoryQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*OrderStatusHistory, init func(*OrderStatusHistory), assign func(*OrderStatusHistory, *Order)) error {
	...
	nodeids := make(map[uuid.UUID][]*OrderStatusHistory)
	for i := range nodes {
		if nodes[i].order_order_status_history == nil {
			continue
		}
		fk := *nodes[i].order_order_status_history
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_order_status_history" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oshq *OrderStatusHistoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(orderstatushistory.Table, orderstatushistory.Columns, sqlgraph.NewFieldSpec(orderstatushistory.FieldID, field.TypeUUID))
	_spec.From = oshq.sql
