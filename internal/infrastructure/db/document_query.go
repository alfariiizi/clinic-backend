...
func (dq *DocumentQuery) QueryClinic() *ClinicQuery {
...
func (dq *DocumentQuery) QueryPatient() *PatientQuery {
...
func (dq *DocumentQuery) WithClinic(opts ...func(*ClinicQuery)) *DocumentQuery {
	query := (&ClinicClient{config: dq.config}).Query()
...
func (dq *DocumentQuery) WithPatient(opts ...func(*PatientQuery)) *DocumentQuery {
	query := (&PatientClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if dq.withClinic != nil || dq.withPatient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withClinic; query != nil {
		if err := dq.loadClinic(ctx, query, nodes, nil,
			func(n *Document, e *Clinic) { n.Edges.Clinic = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withPatient; query != nil {
		if err := dq.loadPatient(ctx, query, nodes, nil,
			func(n *Document, e *Patient) { n.Edges.Patient = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DocumentQuery) loadClinic(ctx context.Context, query *ClinicQuery, nodes []*Document, init func(*Document), assign func(*Document, *Clinic)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
		if nodes[i].clinic_documents == nil {
			continue
		}
		fk := *nodes[i].clinic_documents
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "clinic_documents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DocumentQuery) loadPatient(ctx context.Context, query *PatientQuery, nodes []*Document, init func(*Document), assign func(*Document, *Patient)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
		if nodes[i].patient_documents == nil {
			continue
		}
		fk := *nodes[i].patient_documents
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "patient_documents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
