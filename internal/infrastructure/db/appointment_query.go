...
type AppointmentQuery struct {
	...
	withReminders  *AppointmentReminderQuery
...
func (aq *AppointmentQuery) QueryClinic() *ClinicQuery {
...
func (aq *AppointmentQuery) QueryDoctor() *DoctorQuery {
	query := (&DoctorClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AppointmentQuery) QueryService() *ServiceQuery {
...
func (aq *AppointmentQuery) QueryReminders() *AppointmentReminderQuery {
	query := (&AppointmentReminderClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AppointmentQuery) QueryOrderItems() *OrderItemQuery {
	query := (&OrderItemClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AppointmentQuery) Clone() *AppointmentQuery {
	...
	return &AppointmentQuery{
		...
		withDoctor:     aq.withDoctor.Clone(),
		withService:    aq.withService.Clone(),
		withReminders:  aq.withReminders.Clone(),
		withOrderItems: aq.withOrderItems.Clone(),
...
func (aq *AppointmentQuery) WithClinic(opts ...func(*ClinicQuery)) *AppointmentQuery {
	query := (&ClinicClient{config: aq.config}).Query()
...
func (aq *AppointmentQuery) WithDoctor(opts ...func(*DoctorQuery)) *AppointmentQuery {
	query := (&DoctorClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AppointmentQuery) WithService(opts ...func(*ServiceQuery)) *AppointmentQuery {
	query := (&ServiceClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AppointmentQuery) WithReminders(opts ...func(*AppointmentReminderQuery)) *AppointmentQuery {
	query := (&AppointmentReminderClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withReminders = query
	return aq
}
...
func (aq *AppointmentQuery) WithOrderItems(opts ...func(*OrderItemQuery)) *AppointmentQuery {
	query := (&OrderItemClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withOrderItems = query
	return aq
}
...
//		GroupBy(appointment.FieldAppointmentDate).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.Appointment.Query().
//		Select(appointment.FieldAppointmentDate).
//		Scan(ctx, &v)
...
func (aq *AppointmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Appointment, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			aq.withDoctor != nil,
			aq.withService != nil,
			aq.withReminders != nil,
			aq.withOrderItems != nil,
		}
	)
	if aq.withClinic != nil || aq.withPatient != nil || aq.withDoctor != nil || aq.withService != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withDoctor; query != nil {
		if err := aq.loadDoctor(ctx, query, nodes, nil,
			func(n *Appointment, e *Doctor) { n.Edges.Doctor = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withService; query != nil {
		if err := aq.loadService(ctx, query, nodes, nil,
			func(n *Appointment, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withReminders; query != nil {
		if err := aq.loadReminders(ctx, query, nodes,
			func(n *Appointment) { n.Edges.Reminders = []*AppointmentReminder{} },
			func(n *Appointment, e *AppointmentReminder) { n.Edges.Reminders = append(n.Edges.Reminders, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withOrderItems; query != nil {
		if err := aq.loadOrderItems(ctx, query, nodes,
			func(n *Appointment) { n.Edges.OrderItems = []*OrderItem{} },
			func(n *Appointment, e *OrderItem) { n.Edges.OrderItems = append(n.Edges.OrderItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AppointmentQuery) loadClinic(ctx context.Context, query *ClinicQuery, nodes []*Appointment, init func(*Appointment), assign func(*Appointment, *Clinic)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Appointment)
	for i := range nodes {
		if nodes[i].clinic_appointments == nil {
			continue
		}
		fk := *nodes[i].clinic_appointments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "clinic_appointments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AppointmentQuery) loadDoctor(ctx context.Context, query *DoctorQuery, nodes []*Appointment, init func(*Appointment), assign func(*Appointment, *Doctor)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Appointment)
	for i := range nodes {
		if nodes[i].doctor_appointments == nil {
			continue
		}
		fk := *nodes[i].doctor_appointments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "doctor_appointments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AppointmentQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Appointment, init func(*Appointment), assign func(*Appointment, *Service)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Appointment)
	for i := range nodes {
		if nodes[i].service_appointments == nil {
			continue
		}
		fk := *nodes[i].service_appointments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_appointments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AppointmentQuery) loadReminders(ctx context.Context, query *AppointmentReminderQuery, nodes []*Appointment, init func(*Appointment), assign func(*Appointment, *AppointmentReminder)) error {
	...
	query.withFKs = true
	query.Where(predicate.AppointmentReminder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(appointment.RemindersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.appointment_reminders
		if fk == nil {
			return fmt.Errorf(`foreign-key "appointment_reminders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "appointment_reminders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AppointmentQuery) loadOrderItems(ctx context.Context, query *OrderItemQuery, nodes []*Appointment, init func(*Appointment), assign func(*Appointment, *OrderItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Appointment)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrderItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(appointment.OrderItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.appointment_order_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "appointment_order_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "appointment_order_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
